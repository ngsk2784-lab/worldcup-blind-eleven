...
| 예시 | 접근 | 블라인드 일레븐과의 차이 |
|---|---|---|
| 전술 보드 배치 | 실명 선수를 위치에 배치 | **편견이 개입** → 배치 인터랙션만 신선함(드래그/클릭 모두 가능) |
...
![](../../docs/review/final5/s2-selected-desktop-1440.png)
...
필드 위 11개의 슬롯에 카드를 배치합니다. 선수 카드를 **클릭(탭)하면** 배치 가능한 슬롯이 하이라이트되고, 슬롯을 클릭해 배치합니다.

- **포지션 제약**: 골키퍼 1명 필수, 수비수·미드필더·공격수 간 라인별 정원 제약
- **실시간 피드백**: 빈 슬롯, 포지션 미스매치를 시각적으로 표시해 "감독의 고뇌" 연출
- **조작 직관성**: 선수 클릭 시 유효 슬롯 즉시 하이라이트, 슬롯 클릭 확정 → PC·모바일 동일한 조작감으로 심사위원이 "감독의 선택"을 느낌

11칸이 규칙대로 채워지면 "정체 공개" 버튼이 활성화됩니다. 확정 직전의 순간, 짧은 긴장:

**"지금부터 돌이킬 수 없습니다."**

![](../../docs/review/final5/s2-selected-mobile-390.png)
...
- ✅ **"포지션 제약이 감독의 고뇌"라는 기획** → **클릭/탭 배치 시 라인별 정원 제약 UI로 구현**
...
| **상태관리** | Zustand | 단순·보일러플레이트 최소, 배치 중 리렌더 제어 용이 |
| **배치 인터랙션** | 클릭/탭 모드 | PC·모바일 동일 조작감, 직관적 선택 플로우 |
